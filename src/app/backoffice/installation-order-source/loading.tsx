...
const ERP_SOURCE_LOADING_COLUMN_COUNT = 7;
const ERP_SOURCE_LOADING_COLUMNS = Array.from({ length: ERP_SOURCE_LOADING_COLUMN_COUNT });

export default function InstallOrdersLoading() {
  return (
    <section>
          ...
          <div className="overflow-x-auto">
            <table className="w-max min-w-full border-collapse text-left text-sm whitespace-nowrap">
              <thead className="bg-zinc-50 text-xs font-semibold text-zinc-500">
                <tr>
                  {ERP_SOURCE_LOADING_COLUMNS.map((_, columnIndex) => (
              ...
              <tbody>
                {Array.from({ length: 5 }).map((_, rowIndex) => (
                  <tr key={rowIndex} className="border-b border-zinc-100 last:border-0">
                    {Array.from({ length: ERP_SOURCE_LOADING_COLUMNS.length }).map((__, columnIndex) => (
