...
import QrScanModal from "../reg/QrScanModal";
import SnLocationModal from "../reg/SnLocationModal";
...
export default function AdminRegClient({ initialSn = "" }: { initialSn?: string }) {
  ...
  // 관리자 대행 등록은 기본 동의 처리하여 번거로운 클릭을 줄임
  ...
  // 전화번호가 9자리 이상이면 인증 완료된 상태로 판단 (별도 SMS 인증 우회)
  const isPhoneVerified = useMemo(() => {
    return normalizePhone(userPhone).length >= 9;
  }, [userPhone]);
  ...
  async function onSubmit() {
    ...
    try {
      ...
      sessionStorage.setItem("lastConfirmLink", ""); // 관리자 즉시 완료로 불필요
      sessionStorage.setItem("lastRegistrationStatus", data?.status ?? "confirmed");
