...
/**
 * timing-safe 比对请求提供的 secret 与环境变量 `CAFE24_WEBHOOK_SECRET`。
 *
 * - 未配置 `CAFE24_WEBHOOK_SECRET`：放行并打警告日志（向后兼容，便于灰度上线，
 *   在你把 secret 配好之前不会中断现有付款激活）。
 * - 已配置：缺失或不匹配的 secret 一律返回 false。
 *
 * secret 可以来自 URL 路径段（`/api/cafe24/webhook/<secret>`）——Cafe24 的 webhook
 * 接收 URL 不接受 query string，所以路径段是主要方式；也兼容 `?key=` / header。
 */
export function cafe24SecretMatches(provided: string | null | undefined): boolean {
...
/**
 * 处理已通过 secret 校验的 Cafe24 webhook 请求：确认付款后，把订单里目标商品
 * 的设备 SN 标记为已激活（写入 `device_feature_upgrades`）。
 */
export async function handleCafe24Webhook(req: Request): Promise<NextResponse> {
  try {
    ...
    const isValidEvent =
      (parsedEvent === "order.paid" ||
        parsedEvent === "order.create" ||
        parsedEvent === "create_order") &&
      parsedResourceId;
    if (!isValidEvent) {
      console.log(`[Cafe24 Webhook] Ignored event: ${parsedEvent}`);
      return NextResponse.json({ ok: true, message: "Ignored event" });
    }

    const orderId = parsedResourceId;
    console.log(
      `[Cafe24 Webhook] Processing event ${parsedEvent} for Order ID: ${orderId} (Mall: ${parsedMallId})`,
    );
    ...
    const isPaid =
      order.paid === "T" ||
      (order.items &&
        order.items.some((item: { order_status?: string }) => item.order_status === "N10"));
    if (!isPaid) {
    ...
    for (const item of items) {
      if (item.product_code === TARGET_PRODUCT_CODE) {
        ...
        if (!optionValue) {
          console.error(
            `[Cafe24 Webhook] Target item in Order ${orderId} is missing the device SN option.`,
          );
          continue;
        }
